...
class SemanticFloorTest
    ...
    constructor(
        ...
        /** Mirrors SearchService: one probe per expansion, best score per document wins. */
        private fun semantic(query: String): List<DocumentMatch> {
            val best = LinkedHashMap<UUID, DocumentMatch>()
            expander.expand(query).forEach { probe ->
                documents.semanticSearch(embeddings.embedQuery(probe), CANDIDATES).forEach { match ->
